...
(function() {
  var Neo4jRepostitory, Papa, assert, async, fs, importer, math, should;
  ...
  describe('Errata', function() {
    ...
    checkAllZipCodes = (function(_this) {
      return function(filename1, filename2, repo, done) {
        ...
        for (i = k = 0, len1 = LtlCodes.length; k < len1; i = ++k) {
          ...
          if (math.floor(math.random(0, 10) > 1)) {
            continue;
          }
          for (key in zip3s) {
            zipcode = zip3s[key];
            if (math.floor(math.random(0, 30) > 1)) {
              continue;
            }
            ...
            zipcode.id = id;
            if (id !== "") {
              getZipFuncs.push(make(id, 'LtlCode'));
            }
          }
        }
    ...
    return it('Imports ZipCodes Neo4j', function(done) {
      ...
      importer["import"](filename1, filename2, repo, function(error, results) {
        if ((error != null)) {
          console.log("error: " + error);
          assert(false);
          done();
