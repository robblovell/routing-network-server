...
browserify({standalone: "acorn"})
  ...
  .transform(babelify)
  .require("./src/index.coffee", {entry: true})
  .bundle()
...
function acornShim(file) {
  ...
  if (file == path.resolve(__dirname, "../src/index.coffee")) {
    var sent = false
...
browserify({standalone: "acorn.loose"})
  ...
  .transform(acornShim)
  .transform(babelify)
  .require("./src/loose/index.coffee", {entry: true})
  .bundle()
  .on("error", function (err) { console.log("Error: " + err.message) })
...
browserify({standalone: "acorn.walk"})
  ...
  .transform(acornShim)
  .transform(babelify)
  .require("./src/walk/index.coffee", {entry: true})
  .bundle()
  .on("error", function (err) { console.log("Error: " + err.message) })
